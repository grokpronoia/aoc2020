...
\ts show enlist(*) over'("j"$(1#a where {$[2020 in x;1b;0b]}'[sum each a::(cross/)(tt[;`a];tt[`a])]))

/1.2 - find the 3 numbers in list that equal 2020
-1"Solution 1.2...";
\ts show enlist(*) over'("j"$(1#a where {$[2020 in x;1b;0b]}'[sum each a::(cross/)(tt[;`a];tt[`a];tt[;`a])]))
